-- Migration 0032: Fix batch_abc_classify XYZ computation
-- Replaces synthetic turnover-based CV with actual monthly consumption STDDEV/AVG
-- Previous version (0028) used ABS(turnover * 0.3) / turnover which always = 0.3 -> always 'X'
...
SELECT COALESCE(NULLIF(MAX(inventory_value), 0), 1),
    ...
    LOOP
        ...
        WITH monthly_cons AS (
            SELECT COALESCE(SUM(t.quantity), 0) as cons
            FROM transactions t
            WHERE t.material_id = v_rec.material_id
              AND t.type = 'out'
        ...
        SELECT COALESCE(
            CASE WHEN COUNT(*) > 1 AND AVG(cons) > 0
                THEN STDDEV(cons) / AVG(cons)
                ELSE NULL
            END, 999
        ) INTO v_cv
        FROM monthly_cons;
