    ...
    if body.status == "received" {
        ...
        for (mat_id, qty, to_wh) in items {
            sqlx::query("UPDATE materials SET quantity = GREATEST(COALESCE(quantity,0) - $1, 0) WHERE id=$2")
                .bind(qty).bind(&mat_id).execute(&mut *db_tx).await.map_err(|e| (axum::http::StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": e.to_string()}))))?;
            sqlx::query("UPDATE materials SET quantity = COALESCE(quantity,0) + $1, warehouse_id=$2 WHERE id=$3")
                .bind(qty).bind(&to_wh).bind(&mat_id).execute(&mut *db_tx).await.map_err(|e| (axum::http::StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": e.to_string()}))))?;
        ...
        sqlx::query("UPDATE transfer_orders SET approved_by=$1 WHERE id=$2")
            .bind(&user_id).bind(&id).execute(&mut *db_tx).await.map_err(|e| (axum::http::StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": e.to_string()}))))?;
        db_tx.commit().await.map_err(|e| (axum::http::StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": e.to_string()}))))?;
    }
    Ok(Json(()))
}
