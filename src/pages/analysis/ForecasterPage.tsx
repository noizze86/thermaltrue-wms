...
import { getAnalysisAll, getForecastCache, setForecastCache, deleteForecastCache, forecastGenerate, getForecastSummary, getForecastDetails } from "../../api"
...
const MODEL_LABELS: Record<ModelKey, string> = {
  SMA3: "SMA (n=3)", SMA6: "SMA (n=6)", SMA12: "SMA (n=12)",
  WMA: "WMA", SES: "SES (α)", Holt: "Holt (α+β)",
  HoltWinters: "Holt-Winters (α+β+γ)", LinReg: "Linear Regression",
}
const MODEL_COLORS: Record<ModelKey, string> = {
  SMA3: "#8884d8", SMA6: "#82ca9d", SMA12: "#ffc658", WMA: "#a4de6c",
  SES: "#d0ed57", Holt: "#ff7300", HoltWinters: "#ff0000", LinReg: "#00bcd4",
}
...
function wma(data: number[]): number {
  if (data.length === 0) return 0
  let wSum = 0, sum = 0
  for (let i = 0; i < data.length; i++) { sum += data[i] * (i + 1); wSum += i + 1 }
  return sum / wSum
}
...
function holtWinters(data: number[], alpha: number, beta: number, gamma: number, period = 4): number {
  ...
  for (let i = 1; i < data.length; i++) {
    const si = i % period
    const nl = alpha * (data[i] / season[si]) + (1 - alpha) * (level + trend)
    const nt = beta * (nl - level) + (1 - beta) * trend
    level = nl; trend = nt
    season[(i + 1) % period] = gamma * (data[i] / level) + (1 - gamma) * season[(i + 1) % period]
  }
  return Math.max(0, (level + trend) * season[data.length % period])
}
...
function computeErrorMetrics(actual: number[], predicted: number[]): { mape: number; mae: number; rmse: number } {
  ...
  for (let i = 0; i < actual.length && i < predicted.length; i++) {
    ...
    sumAPE += Math.abs(err / actual[i]) * 100; sumAE += Math.abs(err); sumSE += err * err; count++
  }
  return { mape: count > 0 ? sumAPE / count : 0, mae: count > 0 ? sumAE / count : 0, rmse: count > 0 ? Math.sqrt(sumSE / count) : 0 }
}
...
function runForecast(data: number[], alpha: number, beta: number, gamma: number): ModelResult[] {
  const models: { key: ModelKey; fn: (d: number[]) => number }[] = [
    { key: "SMA3", fn: (d) => sma(d, 3) }, { key: "SMA6", fn: (d) => sma(d, 6) },
    { key: "SMA12", fn: (d) => sma(d, 12) }, { key: "WMA", fn: wma },
    { key: "SES", fn: (d) => ses(d, alpha) }, { key: "Holt", fn: (d) => holt(d, alpha, beta) },
    { key: "HoltWinters", fn: (d) => holtWinters(d, alpha, beta, gamma) }, { key: "LinReg", fn: linearRegression },
  ]
  return models.map(({ key, fn }) => {
    ...
    return { forecast, inSample, ...metrics, key }
  })
}
...
export default function ForecasterPage() {
  ...
  const { data: forecastSummary } = useQuery({ queryKey: ["forecast_summary"], queryFn: getForecastSummary })
  const { data: forecastDetails } = useQuery({ queryKey: ["forecast_details"], queryFn: () => getForecastDetails() })
  ...
  const { history, models, bestModel } = useMemo(() => {
    ...
    let best: ModelKey | null = null; let bestMape = Infinity
    for (const r of results) { if (r.mape > 0 && r.mape < bestMape) { bestMape = r.mape; best = r.key } }
    return { history: hist, models: results, bestModel: best }
  }, [selected, alpha, beta, gamma, periodMonths, cachedForecast])

  const generateMut = useMutation({
    mutationFn: () => forecastGenerate(),
    onSuccess: (res) => {
      queryClient.invalidateQueries({ queryKey: ["forecast_summary"] })
      queryClient.invalidateQueries({ queryKey: ["forecast_details"] })
      toast({ title: "Forecast generated", description: `${res.generated} materials processed` })
    },
  ...
  const clearCache = async () => {
    try {
      ...
      setCacheStatus("Cache cleared"); refetchCache()
  ...
  const chartData = useMemo(() => {
    ...
    history.forEach((v: number, i: number) => {
      ...
      for (const m of models) { if (i > 0 && i - 1 < m.inSample.length) point[m.key] = Math.round(m.inSample[i - 1]) }
      result.push(point)
    })
    const h = parseInt(horizon)
    for (let f = 0; f < h; f++) {
      const fPoint: Record<string, number | string> = { period: `M${history.length + f + 1}` }
      for (const m of models) { fPoint[m.key] = f === 0 ? Math.round(m.forecast) : Math.round(m.forecast * (1 + f * 0.02)) }
      result.push(fPoint)
    }
    return result
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button size="sm" variant="default" onClick={() => generateMut.mutate()} disabled={generateMut.isPending}>
          <BarChart3 className={`mr-1 h-4 w-4 ${generateMut.isPending ? "animate-spin" : ""}`} />
          {generateMut.isPending ? "Generating..." : "Generate Forecast"}
        </Button>
      </div>

      {/* Summary cards from persisted forecast */}
      <div className="grid gap-4 md:grid-cols-4">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm">Forecasted Materials</CardTitle>
            <Package className="h-5 w-5 text-blue-600" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{forecastSummary?.forecasted ?? items?.length ?? 0}</div>
            <p className="text-xs text-muted-foreground">of {forecastSummary?.total_materials ?? items?.length ?? 0} total</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-600">{forecastSummary?.trend_up ?? 0}</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-red-600">{forecastSummary?.trend_down ?? 0}</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-orange-600">{forecastSummary?.avg_mape.toFixed(1) ?? "—"}</div>
            <p className="text-xs text-muted-foreground">Seasonal: {forecastSummary?.seasonal_count ?? 0} items</p>
          </CardContent>
        </Card>
      </div>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>Forecast 1mo</TableHead>
                  <TableHead>Forecast 3mo</TableHead>
                  <TableHead>Forecast 6mo</TableHead>
              ...
              <TableBody>
                {forecastDetails.items.map((item) => (
                  <TableRow key={item.material_id}>
                    <TableCell className="font-medium">{item.material_name}</TableCell>
                    <TableCell className="font-mono">{item.sku}</TableCell>
                    <TableCell>{item.forecast_1mo.toFixed(1)}</TableCell>
                    <TableCell>{item.forecast_3mo.toFixed(1)}</TableCell>
                    <TableCell>{item.forecast_6mo.toFixed(1)}</TableCell>
                    <TableCell>{item.mape.toFixed(1)}%</TableCell>
                    <TableCell>
                      {item.trend === "▲" ? <TrendingUp className="h-4 w-4 text-green-600" /> :
                       item.trend === "▼" ? <TrendingDown className="h-4 w-4 text-red-600" /> :
                       <Minus className="h-4 w-4 text-gray-400" />}
                    </TableCell>
                    <TableCell>
                      <Badge variant={item.is_seasonal ? "default" : "outline"}>
              ...
              <ResponsiveContainer width="100%" height={400}>
                <LineChart data={chartData}>
                  ...
                  {models.map((m: ModelResult) => (
                    <Line key={m.key} type="monotone" dataKey={m.key} stroke={MODEL_COLORS[m.key]} strokeDasharray="4 4" dot={false} connectNulls name={MODEL_LABELS[m.key]} />
                  ))}
                </LineChart>
              </ResponsiveContainer>
            </CardContent>
              ...
              <Table>
                ...
                <TableBody>
                  {models.map((m: ModelResult) => {
                    ...
                    return (
                      <TableRow key={m.key} className={isBest ? "bg-green-50 dark:bg-green-950/30" : ""}>
                        <TableCell className="font-medium">{MODEL_LABELS[m.key]}</TableCell>
